...
public interface ReviewedDao extends PagingAndSortingRepository<Reviewed, Long>{
	...
	//根据审核人查找流程
	@Query("select new cn.gson.oasys.model.entity.process.AubUser(pro.processId,pro.typeName,pro.deeply,pro.processName,pro.userId.userName,pro.applyTime,rev.statusId) "
	...
	//根据类型名和审核人查找流程
	@Query("select new cn.gson.oasys.model.entity.process.AubUser(pro.processId,pro.typeName,pro.deeply,pro.processName,pro.userId.userName,pro.applyTime,rev.statusId) "
			+ "from ProcessList as pro,Reviewed as rev where rev.proId.processId=pro.processId and rev.userId=?1 and pro.typeName=?2 and rev.del=?3 order by rev.statusId")
